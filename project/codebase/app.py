    ...
    with main_content_col:
        ...
        for item_id, item in menu_db.items():
            if search_query and search_query.strip():
                ...
                sq_norm = normalize_text(sq_clean)
                sq_unaccent = remove_accents(sq_clean)
                ...
                match_sq = (
                    sq_norm in name_norm or
                    sq_norm in cat_norm or
                    sq_norm in desc_norm or
                    (bool(sq_unaccent) and (
                        sq_unaccent in remove_accents(item.name) or
                        sq_unaccent in remove_accents(item.category) or
                        sq_unaccent in remove_accents(item.description or "")
                    ))
                )
                if not match_sq:
                    continue
...
with sidebar_tabs[0]:
    ...
    if not cart_items:
        ...
        calc_res = calculate_order(session_id=st.session_state.session_id, voucher_code=voucher_input, shipping_distance_km=dist_km)
        
        if calc_res.get("status") == "success":
            discount_val = calc_res.get("discount_amount", 0)
            shipping_val = calc_res.get("shipping_fee", calc_res.get("delivery_fee", 0))
            total_val = calc_res.get("total_amount", calc_res.get("total", 0))

            if discount_val > 0:
                st.success(f"Khuyến mãi: -{format_vnd(discount_val)}")
            st.write(f"Phí giao hàng: {format_vnd(shipping_val)}")
            st.markdown(f"### Tổng thanh toán: **{format_vnd(total_val)}**")
...
with sidebar_tabs[1]:
    ...
    with sug_c1:
        if st.button("🍱 Gợi ý 100k", key="sug_1"):
            prompt_to_send = "Gợi ý cho mình thực đơn 2 người khoảng 100k"
    with sug_c2:
        if st.button("🎟️ Mã giảm giá", key="sug_2"):
            prompt_to_send = "Hôm nay có mã giảm giá gì hot không?"
    ...
    if prompt_to_send:
        ...
        with st.spinner("OpenAI LLM đang xử lý..."):
            response = run_agent(
                user_id=st.session_state.user_id,
